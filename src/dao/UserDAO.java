...
public class UserDAO 
{
    ...
    {
    	PreparedStatement psmt=conn.prepareStatement("select * from Customers where Login=? and passowrd=?");
    	psmt.setString(1, userObj.getLogin());
    ...
    public boolean insertCustomer(User userObj)throws Exception
    {
    	...
    	PreparedStatement psmt=conn.prepareStatement("insert into Customers values(?,?,?,?,?,?,?)");
    	
    	psmt.setString(1, userObj.getLogin());
    	psmt.setString(2, userObj.getPassword());
    	psmt.setString(3,userObj.getCustomerName());
    	psmt.setString(4, userObj.getEmailId());
    	psmt.setString(5, userObj.getMobileNo());
    	psmt.setString(6, userObj.getAdrress());
